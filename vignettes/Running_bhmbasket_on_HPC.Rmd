...
With a parallel backend registered as shown above, the job scheduler will automatically distribute the parallel jobs to the worker nodes via `plan(slurm)`, and with the nested parallelization built into `performAnalyses()`, each worker node makes use of its CPUs via `plan(multisession)`.

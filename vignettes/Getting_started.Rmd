...
The package provides printing functions to print the outputs of the analysis list, the scenario list, and the decision list.
...
Also actual trial outcomes can be analyzed with bhmbasket.
...
Target rates are used to derive default prior parameters and are part of some analysis models.
For some models do not require target rates, prior parameters can be specified directly with `prior_parameters_list`.
...
The resulting decision list can be used to calculate the cohort-wise and overall Go decision probabilities using the functions [`getGoProbabilities()`](https://search.r-project.org/CRAN/refmans/bhmbasket/html/getGoProbabilities.html).
Optional, adding a NoGo decision list using [`negateGoDecisions()`](https://search.r-project.org/CRAN/refmans/bhmbasket/html/negateGoDecisions.html), the function `getGoProbability()` provides additional NoGo and Consider probabilities.

Calling the printing function of the decision list leads automatically to the execution of `getGoProbabilities()` and the output of the Go probabilities.
...
$$q_{1-\gamma_j} > p_{Bj}$$
As keeping the MCMC samples or marginal posterior densities of the parameters of interest would require too much memory, only pre-specified quantiles of the MCMC samples are stored when applying `performAnalyses()`.
...
In this example, the more borrowing, the smaller the false Go probability in the negative scenario.
The model proposed by Berry et al. (2013) leads to the highest amount of borrowing and therefore, to the best outcome.
In general, using *ExNex adjusted* results in less borrowing of information compared to *berry*.
Using the method *Stratified* yields the highest false Go probabilities, which can be attributed to the absence of borrowing.
...
```{r}
getAverageNSubjects(final_scenario_list)
```

...
This example consists of four scenarios and three cohorts, whereby the third cohort is historical with an observed number of five responders out of 15 subjects.
...
if (file.exists("analyses_lists/gettingStarted_analysis_1.rds")) {
  ...
  analysis_list <- readRDS("analyses_lists/1gettingStarted_analysis_1.rds")
  ...
  saveRDS(analysis_list, "analyses_lists/1gettingStarted_analysis_1.rds")
}
...
## check for unnecessary data
  ...
  analysis_list$scenario_1$scenario_data$previous_analyses$post_quantiles <- NULL
  analysis_list$scenario_2$scenario_data$previous_analyses$post_quantiles <- NULL
  analysis_list$scenario_3$scenario_data$previous_analyses$post_quantiles <- NULL
  analysis_list$scenario_4$scenario_data$previous_analyses$post_quantiles <- NULL
...
This makes this approach very flexible for implementing many different and potentially comlex decision rules.
...
Optionally providing a NoGo decision list, `getGoProbability()` calculates also Consider and NoGo decision probabilities.
...
decision_list_adj
```
In the negative scenario, the false Go probabilities of the method Stratified are comparatively high with an overall Go probability of 55%, and 16% and 46% cohort-wise false go probability in Cohorts 1 and 2, respectively.
Using the model ExNex adjusted leads to false Go probabilities of 16% in Cohort 1 and 23% in Cohort 2.
The false Go probability of Cohort 1, using the model proposed by @berry2013, is 6 percentage points lower compared to the probability when using the model ExNex adjusted.
...
if (file.exists("analyses_lists/gettingStarted_analysis_2.rds")) {
  ...
  final_analysis_list <- readRDS("analyses_lists/2gettingStarted_analysis_2.rds")
...
```{r, include = FALSE}
## check for unnecessary data
if (!is.null(final_analysis_list$scenario_1$scenario_data$n_responders)) {  
  ## remove unnecessary data
  
  ## quantiles_list
  # final_analysis_list$scenario_1$quantiles_list$exnex_adj <-   lapply(analyses_futility_1$scenario_1$quantiles_list$exnex_adj, function   (x) {
  #   x[-c(2:4, 6), -c(1:3)]
  # })
  # final_analysis_list$scenario_2$quantiles_list$exnex_adj <-   lapply(analyses_futility_1$scenario_1$quantiles_list$exnex_adj, function   (x) {
  #   x[-c(2:4, 6), -c(1:3)]
  # })
  # final_analysis_list$scenario_3$quantiles_list$exnex_adj <-   lapply(analyses_futility_1$scenario_1$quantiles_list$exnex_adj, function   (x) {
  #   x[-c(2:4, 6), -c(1:3)]
  # })
  # final_analysis_list$scenario_4$quantiles_list$exnex_adj <-   lapply(analyses_futility_1$scenario_1$quantiles_list$exnex_adj, function   (x) {
  #   x[-c(2:4, 6), -c(1:3)]
  # })
  # 
  
  ##subjects, responders, post_quantiles and prior_parameters
  final_analysis_list$scenario_1$scenario_data$n_subjects <- NULL
  final_analysis_list$scenario_2$scenario_data$n_subjects <- NULL
  final_analysis_list$scenario_3$scenario_data$n_subjects <- NULL
  final_analysis_list$scenario_4$scenario_data$n_subjects <- NULL
  
  final_analysis_list$scenario_1$scenario_data$n_responders <- NULL
  final_analysis_list$scenario_2$scenario_data$n_responders <- NULL
  final_analysis_list$scenario_3$scenario_data$n_responders <- NULL
  final_analysis_list$scenario_4$scenario_data$n_responders <- NULL
  
  final_analysis_list$scenario_1$scenario_data$previous_analyses$post_quantiles <- NULL
  final_analysis_list$scenario_2$scenario_data$previous_analyses$post_quantiles <- NULL
  final_analysis_list$scenario_3$scenario_data$previous_analyses$post_quantiles <- NULL
  final_analysis_list$scenario_4$scenario_data$previous_analyses$post_quantiles <- NULL
...
As an example, decision rules applying the relevance rule $P(p_1|\text{data} > 0.8) > 0.5$ and the significance rule $P(p_1|\text{data} > 0.4) > 0.95$ can be implemented as follows:
```{r, eval=FALSE}
cohort_names    = c('p_1', 'p_2',
                    'p_1', 'p_2')
...
For Cohort 2, the relevance criterion $P(p_2|\text{data} > 0.28) > 0.5$ ∧ $P(p_2-p_1|\text{data} > 0) > 0.5 $ is a combination of rules for the point estimate and for the difference between Cohort 2 and Cohort 1.
The rule for significance for Cohohrt 2 is $P(p_2|\text{data} > 0.15) > 0.8$.
...
```
In this case, the specified decision rules lead to an overall Go probability of 88% and individual Go probabilities of 68% in Cohort 1 and 70% in Cohort 2 in the positive scenario. 
The probabilities of identifying the nugget cohorts are 60% and 65%, respectively.
In the negative scenario, the overall false Go probability is 11%.

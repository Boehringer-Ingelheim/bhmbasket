...
```{r}
go_decisions_futility <- getGoDecisions(
  analyses_list   = analyses_futility_1,
  cohort_names    = "p_1",
  evidence_levels = 0.5,
  boundary_rules  = quote(c(x[1] >= 0.2, TRUE, TRUE)))

go_probability_futility <- getGoProbabilities(go_decisions_futility_1)
scaleRoundList(go_probability_futility, round_digits = 2)
```
...
Thus no borrowing between the cohorts is required and the model *stratified* is used.
...
The sets of decision rules lead to an overall Go probability of xx% and individual Go probabilities of 71% in Cohort 1. 
The probabilities of identifying the nugget cohort is 73%.
In the negative scenario, the overall Go probability is 3%.
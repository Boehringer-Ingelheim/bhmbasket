...
The trial consists of four cohorts with a planned total of 70, 25, 25, and 25 subjects.
...
The setup of the trial design starts with the simulation of the scenario data with the function `simulateScenarios()`.
...
The cohorts will be simulated in three different scenarios, a positive, a nugget, and a negative scenario. 
...
## simulation
scenarios_interim_1 <- simulateScenarios(
...
The first analysis follows directly after the simulation.
Therefore, the function `performAnalyses()` analyses all cohorts (except Cohort 4) with 10 000 MCMC iterations.
...
```{r, message=FALSE, warning=FALSE}
analyses_interim_1 <- performAnalyses(
  scenario_list     = scenarios_interim_1,
  method_names      = "exnex_adj",
  target_rates      = orr_target_rates[-4])
```

One can get point estimates and credible intervals per cohorts, as well as estimated biases and mean squared errors of the point estimators using the function `getEstimates(analyses_interim_1)`.

#### Decision Making

The created analyses list is now used to make Go/NoGo decisions as part of the interim analysis.
...
```{r}
go_decisions_interim_1 <- getGoDecisions(
  analyses_list   = analyses_interim_1,
  cohort_names    = "p_1",
  evidence_levels = 0.5,
  boundary_rules  = quote(c(x[1] >= 0.2, TRUE, TRUE)))

go_probability_interim <- getGoProbabilities(go_decisions_interim_1)
scaleRoundList(go_probability_interim, round_digits = 2)
```

##### SuperGo

After the first Go/NoGo decision making, a SuperGo decision making will be performed with different decision boundaries.
The additional SuperGo outcome can be used for a potential accelerated approval or to skip directly to the next Phase.
In this case, Cohort 1 will have the next Phase directly after reaching a SuperGo in the futility analysis without being analysed in a second analysis.
The boundary rules of the SuperGo decision are implemented in the same way as above: 

$P(p_1|\text{data} > 0.6) > 0.8$
```{r}
...
go_probability_SuperGo <- getGoProbabilities(go_decisions_SuperGo)
scaleRoundList(go_probability_SuperGo, round_digits = 2)
```
...
To make sure that there are not more SuperGos than Gos in this two decision lists, a sanity check is performed.

```{r}
## sanity check: go probability in PoCP always greater than or equal to SuperGo probability
all(
  sapply(seq_along(scenarios_interim_1), function (i) getGoProbabilities(go_decisions_interim_1)[[1]][[i]])[1, ] >=
    sapply(seq_along(scenarios_interim_1), function (i) getGoProbabilities(go_decisions_SuperGo)[[1]][[i]])[1, ])
```

### Add new Cohort

After Cohort 1 passed the interim analysis, Cohort 4 will be included.
Therefore, the decision list must be manipulated.
This includes adding another cohort as well as decisions for this cohort, which are necessary for the subsequent continuation of recruitment. 
The decision list will be copied in `go_decisions_1` and adjusted for each scenario. 

```{r}
go_decisions_1 <- go_decisions_interim_1
```

The first step is to combine the overall decision and the decisions for Cohort 1, 2, and 3 with a decision for Cohort 4.
...
go_decisions_1[[1]]$decisions_list$exnex_adj <-
    cbind(go_decisions_1[[1]]$decisions_list$exnex_adj[, c(1, 2, 3, 4)],
          "decision_4" = go_decisions_interim_1[[1]]$decisions_list$exnex_adj[, 2])
...
go_decisions_1[[1]]$scenario_data$n_subjects <- 
    cbind(go_decisions_interim_1[[1]]$scenario_data$n_subjects[, c(1, 2, 3)],
          "n_4" = 0)
...
for (s in seq_along(orr_scenario_list)) {
  ...
  go_decisions_1[[s]]$scenario_data$n_responders <- 
    cbind(go_decisions_interim_1[[s]]$scenario_data$n_responders[, c(1, 2, 3)],
          "r_4" = 0)
...
The precondition for this recruitment is a SuperGo in the futility analysis or a Go in the primary analysis II.
...
The filtered decision list is used for continuation of recruitment fulfilling the aforementioned precondition.
...
**Include?**  
If one is interested in the mean number of subjects per scenario, one can get an overview with following function:
```{r}
## mean number of subjects per scenario in Phase 2
lapply(scenarios_3, function (x) {
  colMeans(x$n_subjects)
})
```
...
Therefore, the model *stratified* is used, due to no required borrowing between the cohorts.

```{r, message=FALSE, warning=FALSE}
analyses_3 <- performAnalyses(
  scenario_list         = scenarios_3,
  method_names          = "stratified",
  target_rates          = orr_target_rates)
```

#### Decision Making

The final decision rules for Cohort 1 are proposed by [Fisch et al. (2015)](https://link.springer.com/article/10.1177/2168479014533970).
Further information on how to apply these rules proposed by Fisch et al. are available in the vignette [Getting started](...) of the bhmbasket package.
The decision rules are based on relevance and significance and are implemented as follows:  

Significance: $P(p_1|\text{data} > 0.3) > 0.95$  
Relevance:    $P(p_1|\text{data} > 0.6) > 0.5$ 
...
go_decisions_3  <- getGoDecisions(
  ...
  boundary_rules  = quote(c(x[1] >= 0.3 & x[2] > 0.15, TRUE, TRUE, TRUE)),
  overall_min_gos = 4)
